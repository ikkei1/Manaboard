...
export function StudyForm({ initial, onSubmit, submitting }: { initial?: Partial<StudyInput>; onSubmit: (data: StudyInput) => Promise<void>; submitting?: boolean }) {
  const [form, setForm] = useState<StudyInput>({ subject: initial?.subject ?? "数学", study_minutes: initial?.study_minutes ?? 30, studied_at: initial?.studied_at ?? todayString(), memo: initial?.memo ?? "" });
