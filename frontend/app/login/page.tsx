...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "ログインできませんでした");
    }
  }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <form className="panel grid w-full max-w-md gap-4" onSubmit={submit}>
        <div>
          <h1 className="text-2xl font-bold">Manaboard</h1>
          <p className="text-sm text-slate-600">学習の記録を始めましょう</p>
        </div>
        ...
        <Link className="text-center text-sm font-semibold text-focus" href="/register">
          アカウントを作成
        </Link>
      </form>
    </main>
