...
export default function RegisterPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "登録できませんでした");
    }
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <form className="panel grid w-full max-w-md gap-4" onSubmit={submit}>
        <div>
          ...
          <p className="text-sm text-slate-600">Manaboardへようこそ</p>
        </div>
        {error && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}
        <label className="grid gap-1 text-sm font-semibold">
          ユーザー名
          <input className="field" maxLength={30} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required />
        </label>
        <label className="grid gap-1 text-sm font-semibold">
          メールアドレス
        ...
        <Link className="text-center text-sm font-semibold text-focus" href="/login">
          ログインへ戻る
        </Link>
      </form>
    </main>
